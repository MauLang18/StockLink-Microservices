...
namespace StockLink.Compra.Application.UseCase.Services
{
    public class PublisherHubService : IPublisherHubService
    {
        ...
        public async Task SendNotification(int Id,string CodigoCliente, string Cliente, string CodigoArticulo, string Articulo, string Vendedor, int Cantidad, int EstadoPedido, DateTime FechaPedido, string Observacion)
        {
            _logger.LogInformation($"Pedido para {CodigoCliente} enviado, observacion {Observacion}");
            await _publishEndpoint.Publish(new PedidoHubRecord(Id, CodigoCliente, Cliente, CodigoArticulo, Articulo, Vendedor, Cantidad, EstadoPedido, FechaPedido, Observacion));
        }
    }
}